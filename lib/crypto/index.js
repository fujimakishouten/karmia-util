...
class KarmiaUtilityCrypto {
    ...
    constructor(options) {
        ...
        self.options.iv.type = self.options.iv.type || 'memory';
        ...
        if (_.isObject(self.options.iv.type)) {
            self.initial_vector = new self.options.iv.type(self.options.iv);
        } else {
            self.initial_vector = new iv[self.options.iv.type](self.options.iv);
        }
    }
    ...
    iv(callback) {
        const self = this;
        if (!callback) {
            return self.initial_vector.get();
        }

        self.initial_vector.get(callback);
    }

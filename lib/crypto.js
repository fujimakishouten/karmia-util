...
module.exports = {
    ...
    encrypt: function (algorythm, password, data, encoding) {
        const result = {},
            secret = Buffer.isBuffer(password) ? password : new Buffer(password, 'binary'),
        ...
        result.data = new Buffer(encrypted, 'binary');
    ...
    decrypt: function (algorythm, password, data, encoding) {
        const secret = Buffer.isBuffer(password) ? password : new Buffer(password, 'binary'),
            decipher = crypto.createDecipher(algorythm, secret),
            encrypted = _.has(data, 'data') ? data.data : data,
            buffer = _.isBuffer(encrypted) ? encrypted : new Buffer(encrypted, encoding);

        return new Buffer(decipher.update(buffer, encoding, 'binary') + decipher.final('binary'), 'binary');
    },
